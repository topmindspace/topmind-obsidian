...
export function appendProfileEntry({ workspaceRoot, entry, contract, actor, confirmed, writebackModeOverride, src }) {
  ...
  // Live-section consolidation (never blind append of the same fact):
  // - exact same fact → skip
  // - containment-grade near-dup (one fact is the other plus/minus a few words)
  //   → fuse in place to the newest wording (one live line, latest truth)
  // - softer Jaccard similarity → append (may be a distinct fact, e.g. numbered
  //   details); periodic organize offers a confirm-gated fuse for those pairs
  // History is not live — re-appending a retired fact is allowed (re-activation).
  const near = findLiveFactConflictsInBody(body, content, { threshold: PROFILE_MERGE_THRESHOLD, limit: 1 });
  if (near.length > 0) {
    const hit = near[0];
    if (hit.score >= 1) {
      return {
    ...
    if (hit.score >= PROFILE_FUSE_THRESHOLD && isContainmentNearDupe(hit.text, content)) {
      // Same fact, new wording → update the live line to the newest fact.
      return updateProfileEntry({
        workspaceRoot,
        match: hit.text,
        content,
...
function findProfileFactLineIndexes(sectionBody, match) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Fuzzy fallback: paraphrased match text still addresses the fact.
    const score = factSimilarity(lines[i], match);
    if (score >= PROFILE_MERGE_THRESHOLD) fuzzy.push({ i, score });
  }
  if (hits.length > 0) return hits;
  if (fuzzy.length > 0) {
    fuzzy.sort((a, b) => b.score - a.score);
    // Best hit only — never fan out a paraphrase across several facts.
    return [fuzzy[0].i];
  }
  return [];
}
...
export function retireProfileEntry({ workspaceRoot, match, section, historySection, contract, actor, confirmed, writebackModeOverride, reason }) {
  ...
  const matchKey = normalizeProfileFactKey(target);
  const hitPlain = (removed[0] ? parseFactMeta(removed[0]).text : "") || "";
  const hitKey = normalizeProfileFactKey(hitPlain);
  return {
    ...evidence,
    matchedText: hitPlain,
    matchExact: matchKey === hitKey,
    matchScore: matchKey === hitKey ? 1 : factSimilarity(target, hitPlain),
  };
}
...
export function updateProfileEntry({ workspaceRoot, match, content, section, historySection, contract, actor, confirmed, writebackModeOverride }) {
  ...
  // Match diagnostics: UIs show the fact actually rewritten when `match` was
  // a paraphrase (fuzzy) rather than the exact bullet text.
  const matchKey = normalizeProfileFactKey(target);
  const hitKey = normalizeProfileFactKey(oldMeta.text || "");
  return {
    ...evidence,
    matchedText: oldMeta.text || "",
    matchExact: matchKey === hitKey,
    matchScore: matchKey === hitKey ? 1 : factSimilarity(target, oldMeta.text || ""),
  };
}
...
`;
  ...
  // Topic-memory dedupe (same gate as profile): never stack a second copy of
  // the same / near-same fact under the topic home.
  const topicKey = normalizeProfileFactKey(content);
  if (topicKey) {
    for (const line of body.split("\n")) {
      ...
      if (existing === topicKey) {
        return {
          ...
          targetPath: `${memoryDirRel(workspaceRoot)}/topics/${slug}.md`,
          target_path: topicPath,
          note: "topic fact already present (deduped)",
          reason: "duplicate-fact",
        };
      }
      if (existing.length >= 6 && topicKey.length >= 6 && (existing.includes(topicKey) || topicKey.includes(existing))) {
        return {
          operation: "skip",
          wroteFiles: false,
          wrote_files: false,
      ...
      if (factSimilarity(line, content) >= PROFILE_FUSE_THRESHOLD && isContainmentNearDupe(line, content)) {
        return {
          operation: "skip",
          wroteFiles: false,
          wrote_files: false,
          targetPath: `${memoryDirRel(workspaceRoot)}/topics/${slug}.md`,
          target_path: topicPath,
          note: "topic near-duplicate fact already present (merged to existing)",
          reason: "near-duplicate-fact",
        };
      }
    }
  }
...
export function factSimilarity(a, b) {
  ...
  // Latin tokens get a light suffix fold so "learning" and "learn" share a key —
  // word-order / inflection paraphrases still score above the 0.72 merge floor.
  const foldLatin = (w) => {
    // Irregular pairs first (conservative map — never invent stems).
    const irregular = {
      prefers: "prefer",
      preference: "prefer",
      preferences: "prefer",
  ...
  const tokenize = (s) => {
    ...
    for (const m of s.matchAll(/[a-z0-9]+|[一-鿿]/gu)) {
      const raw = m[0];
      // Latin: fold suffixes so "learning"/"learn" collapse to one key.
      tokens.add(/[a-z]/u.test(raw) ? foldLatin(raw) : raw);
    }
...
// ── Consolidation (merge / fuse, never blind append) ────────────────────────

/** Near-dupe score floor for "same fact, different wording" (confirm cards). */
export const PROFILE_MERGE_THRESHOLD = 0.72;

/**
 * Auto-fuse floor on append: only containment-grade near-dups (0.92+) rewrite
 * the live line in place. Softer similarity (0.72–0.91) may be a genuinely
 * different fact (e.g. numbered details) — it still appends, and
 * `memory_organize` / `findIntraProfileNearDups` offer a confirm-gated fuse.
 */
export const PROFILE_FUSE_THRESHOLD = 0.92;

/**
 * True when `a`/`b` are the same fact at containment grade: exact key, or one
 * normalized key contains the other. Pure Jaccard overlap (shared boilerplate
 * on "条目 0/1/2") must NOT auto-fuse — those are distinct facts.
 * @param {string} a
 * @param {string} b
 * @returns {boolean}
 */
export function isContainmentNearDupe(a, b) {
  const ka = normalizeProfileFactKey(a);
  const kb = normalizeProfileFactKey(b);
...
/**
 * Scan a profile body (not disk) for live facts near-dup to `candidate`.
 * @param {string} body
 * @param {string} candidate
 * @param {{ threshold?: number, limit?: number, includeHistory?: boolean }} [opts]
 * @returns {Array<{ text: string, fid: string|null, section: string, score: number, line: string, isHistory: boolean }>}
 */
export function findLiveFactConflictsInBody(body, candidate, opts = {}) {
  const threshold = Number.isFinite(opts.threshold) ? Number(opts.threshold) : PROFILE_MERGE_THRESHOLD;
  const limit = Number.isFinite(opts.limit) ? Number(opts.limit) : 3;
  const key = normalizeProfileFactKey(candidate);
  if (!key) return [];
  const inv = listProfileFactsFromBody(body);
  const hits = [];
  for (const sec of inv.sections) {
    if (sec.isHistory && opts.includeHistory !== true) continue;
    for (const f of sec.facts) {
      const score = normalizeProfileFactKey(f.text) === key ? 1 : factSimilarity(f.text, candidate);
      if (score >= threshold) {
        hits.push({ text: f.text, fid: f.fid, section: sec.title, score, line: f.line, isHistory: sec.isHistory });
      }
    }
  }
  hits.sort((a, b) => b.score - a.score);
...
/**
 * Intra-profile near-dup pairs among LIVE facts (for periodic consolidate).
 * Each pair reported once (older first). @see listProfileFacts
 * @param {string} workspaceRoot
 * @param {{ threshold?: number, contract?: object, limit?: number }} [opts]
 * @returns {Array<{ keep: object, merge: object, score: number }>}
 */
export function findIntraProfileNearDups(workspaceRoot, opts = {}) {
  const threshold = Number.isFinite(opts.threshold) ? Number(opts.threshold) : PROFILE_MERGE_THRESHOLD;
  const limit = Number.isFinite(opts.limit) ? Number(opts.limit) : 5;
  const inv = listProfileFacts(workspaceRoot, { contract: opts.contract });
  /** @type {Array<{ text: string, key: string, date: string|null, fid: string|null, section: string }>} */
  const live = [];
  for (const sec of inv.sections) {
    if (sec.isHistory) continue;
    for (const f of sec.facts) {
      live.push({ text: f.text, key: f.key, date: f.date, fid: f.fid, section: sec.title, line: f.line });
    }
  }
  const pairs = [];
  for (let i = 0; i < live.length; i++) {
    for (let j = i + 1; j < live.length; j++) {
      ...
      const score = a.key === b.key ? 1 : factSimilarity(a.text, b.text);
      if (score < threshold) continue;
      // Newer date wins as the surviving fact; undated loses to dated.
      const da = a.date || "";
      const db = b.date || "";
      const keep = db > a.date || (!da && db) ? b : da > db ? a : a; // tie → keep first
      const merge = keep === a ? b : a;
      pairs.push({ keep, merge, score });
    }
  }
...
/**
 * Collapse near-duplicate HISTORY lines, keeping the newest archived wording.
 * Live sections are untouched. Returns write evidence (or skip when nothing to do).
 * @param {object} options
 * @param {string} options.workspaceRoot
 * @param {object} [options.contract]
 ...
 * @returns {object} write evidence
 */
export function compactProfileHistory({ workspaceRoot, contract, actor, confirmed, writebackModeOverride, threshold }) {
  const profilePath = resolveMemoryLayerPath(workspaceRoot, "global");
  const body = readProfileBody(workspaceRoot);
  if (!body) {
    return { operation: "skip", wroteFiles: false, wrote_files: false, targetPath: globalProfileRelPath(workspaceRoot), target_path: profilePath, note: "profile.md not found", reason: "no-profile" };
  ...
  const historyTitles = new Set(PROFILE_SECTION_ALIASES.history);
  historyTitles.add(historyTitle);
  const floor = Number.isFinite(threshold) ? Number(threshold) : PROFILE_MERGE_THRESHOLD;
  const blocks = parseProfileSections(body);
  const hist = blocks.find((b) => b.title != null && historyTitles.has(b.title));
  if (!hist) {
    return { operation: "skip", wroteFiles: false, wrote_files: false, targetPath: globalProfileRelPath(workspaceRoot), target_path: profilePath, note: "no history section", reason: "no-history" };
  }
  const lines = hist.body.split("\n");
  /** @type {Array<{ idx: number, key: string, date: string|null, text: string }>} */
  const facts = [];
  for (let i = 0; i < lines.length; i++) {
    if (!/^\s*[-*+]\s+\S/u.test(lines[i])) continue;
    const meta = parseFactMeta(lines[i]);
    if (!meta.text) continue;
    const dateMatch = meta.text.match(/^[（(](\d{4}-\d{2}-\d{2})/u);
    facts.push({ idx: i, key: normalizeProfileFactKey(meta.text), date: dateMatch ? dateMatch[1] : null, text: meta.text });
  }
  if (facts.length < 2) {
    return { operation: "skip", wroteFiles: false, wrote_files: false, targetPath: globalProfileRelPath(workspaceRoot), target_path: profilePath, note: "history already compact", reason: "no-duplicates" };
  }
  const drop = new Set();
  for (let i = 0; i < facts.length; i++) {
    if (drop.has(facts[i].idx)) continue;
    for (let j = i + 1; j < facts.length; j++) {
      if (drop.has(facts[j].idx)) continue;
      const score = facts[i].key === facts[j].key ? 1 : factSimilarity(facts[i].text, facts[j].text);
      if (score < floor) continue;
      // Keep the newer archived line; drop the older near-dup.
      const di = facts[i].date || "";
      const dj = facts[j].date || "";
      const dropIdx = dj > di ? facts[i].idx : facts[j].idx;
      drop.add(dropIdx);
    }
  }
  if (drop.size === 0) {
  ...
  // Rebuild body with compacted history block.
  const before = body.split("\n");
  const slice = before.slice(hist.start, hist.end);
  const compacted = slice.filter((_, i) => !drop.has(i));
  const nextLines = [
    ...before.slice(0, hist.start),
    ...compacted,
    ...before.slice(hist.end),
  ];
  ...
  const evidence = executeWrite({
    ...
    contract,
    operation: "update",
    ...memoryWriteGate({ actor, confirmed: confirmed !== false }),
    skipShadow: true,
    role: "memory",
    writebackModeOverride,
  });
  if (evidence.wroteFiles !== false) {
    appendMemoryJournal(workspaceRoot, {
      op: "compact-history",
      section: historyTitle,
      note: `dropped ${drop.size} near-duplicate history line(s)`,
      actor: actor || "ai",
    });
  }
  return { ...evidence, dropped: drop.size };
}

/**
 * listProfileFacts against an in-memory body (no disk read).
 * @param {string} body
 */
function listProfileFactsFromBody(body) {
  const locale = resolveAiLocale(undefined);
  const historyTitle = resolveProfileSectionTitle(body, "history", locale);
  const historyTitles = new Set(PROFILE_SECTION_ALIASES.history);
  historyTitles.add(historyTitle);
  const sections = [];
  for (const block of parseProfileSections(body)) {
    ...
    for (const line of block.body.split("\n")) {
      ...
      const meta = parseFactMeta(line);
      if (!meta.text) continue;
      facts.push({ text: meta.text, key: normalizeProfileFactKey(meta.text), fid: meta.fid, line });
    }
    sections.push({ title: block.title, isHistory, facts });
  }
  return { sections };
}
