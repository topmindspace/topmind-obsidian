...
4) Update candidates — facts ALREADY in the existing profile whose meaning changed (quote match from the profile; content is the corrected fact; max 3; empty array if none). Do not also put the same fact in profile or retire.
...
4) 原位更新候选——上方已有画像中含义已变的条目（match 引用画像原文；content 为纠正后的事实；最多 3 条；没有则空数组）。同一条不要同时放进 profile 或 retire。
...
用中文输出内容。`;
    ...
    for (const item of profile) {
      ...
      // update card (replace), not a silent second live line.
      let conflicts = [];
      try {
        conflicts = findConflictingProfileFacts(ctx.workspaceRoot, cleanLine, { threshold: 0.72, limit: 2 });
      } catch { conflicts = []; }
      if (conflicts.length > 0 && conflicts[0].score >= 0.92) {
        suggestions.push({
